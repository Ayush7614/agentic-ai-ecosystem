#!/usr/bin/env python3
"""Render OpenCode Masterclass GIFs."""
...
DIAGRAMS = {
    "opencode-stack": "diagram-opencode-stack.gif",
    "build-plan": "diagram-build-plan.gif",
    "mcp-tools": "diagram-mcp-tools.gif",
}
STEPS = [
    ("01-install-connect", "step-01-install-connect"),
    ("02-init-agents", "step-02-init-agents"),
    ("03-plan-build", "step-03-plan-build"),
    ("04-at-reference", "step-04-at-reference"),
    ("05-share-undo", "step-05-share-undo"),
]
...
def main():
    ...
    if cmd in ("all", "diagrams"):
        for did, name in DIAGRAMS.items():
            print(f"GIF {name} …")
            gif(ROOT / "opencode-diagrams.html", ROOT / name, f"?diagram={did}&frame={{p}}")
    if cmd in ("all", "terminal"):
        for sid, name in STEPS:
            print(f"GIF {name}.gif …")
            gif(ROOT / "opencode-terminal.html", ROOT / f"{name}.gif", f"?step={sid}&frame={{p}}", 980, 560, 18, 10)
    if cmd in ("all", "mega"):
        print("GIF mega-opencode-everything.gif …")
        gif(ROOT / "opencode-mega.html", ROOT / "mega-opencode-everything.gif", "?frame={p}", 1200, 600, 48, 12)
    print("Done.")
