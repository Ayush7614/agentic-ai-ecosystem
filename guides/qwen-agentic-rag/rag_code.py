...
COLLECTION_NAME = "ml_faq_collection"
...
class EmbedData:
    ...
    def embed(self, contexts):
        ...
        for batch_context in tqdm(
            ...
            total=max(len(contexts) // self.batch_size, 1),
...
class QdrantVDB:
    def __init__(
        ...
        storage_path = path or os.getenv("QDRANT_PATH")
        if storage_path:
    ...
    def ingest_data(self, embeddata: EmbedData):
        for batch_context, batch_embeddings in tqdm(
            ...
            total=max(len(embeddata.contexts) // self.batch_size, 1),
