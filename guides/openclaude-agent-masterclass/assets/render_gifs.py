#!/usr/bin/env python3
"""Render OpenClaude Masterclass GIFs."""
...
DIAGRAMS = {
    "openclaude-stack": "diagram-openclaude-stack.gif",
    "provider-profiles": "diagram-provider-profiles.gif",
    "slash-commands": "diagram-slash-commands.gif",
    "agent-routing": "diagram-agent-routing.gif",
}
STEPS = [
    ("01-install-provider", "step-01-install-provider"),
    ("02-first-task", "step-02-first-task"),
    ("03-init-permissions", "step-03-init-permissions"),
    ("04-diff-rewind", "step-04-diff-rewind"),
    ("05-bg-session", "step-05-bg-session"),
]
...
def main():
    ...
    if cmd in ("all", "diagrams"):
        for did, name in DIAGRAMS.items():
            print(f"GIF {name} …")
            gif(ROOT / "openclaude-diagrams.html", ROOT / name, f"?diagram={did}&frame={{p}}")
    if cmd in ("all", "terminal"):
        for sid, name in STEPS:
            print(f"GIF {name}.gif …")
            gif(ROOT / "openclaude-terminal.html", ROOT / f"{name}.gif", f"?step={sid}&frame={{p}}", 980, 560, 18, 10)
    if cmd in ("all", "mega"):
        print("GIF mega-openclaude-everything.gif …")
        gif(ROOT / "openclaude-mega.html", ROOT / "mega-openclaude-everything.gif", "?frame={p}", 1200, 600, 48, 12)
    print("Done.")
